...
for i in (closedList) :
    ...
    WpList = [carte.graph.listOfNodes[i].x, carte.graph.listOfNodes[i].y]

    WPlist.append(WpList)
...
for t in (simu.t):
    ...
    testPassage = WPManager.distanceToCurrentWP (robot.x,robot.y)

    robotDeplacementX.append(robot.x)

    robotDeplacementY.append(robot.y)

    if (testPassage < WPManager.epsilonWP):
...
plt.xlabel('x (m)')
plt.ylabel('y (m)')
robotBody, = plt.plot(robotDeplacementX, robotDeplacementY, 'o-', lw=1)
